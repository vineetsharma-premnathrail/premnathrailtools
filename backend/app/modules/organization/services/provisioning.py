...
`department`, and `reporting_manager_id` fields are set from Graph data:
  ...
  - `office_location` -> matched/created as a Branch under the single
    company, and linked onto the user via `User.branch_id`.
...
This app is single-company, so "the company" is simply the first row in
`companies` — if none exists yet (Organization > Company not set up), this
is a no-op rather than guessing at company data.
"""
...
def _get_or_create_branch(db: Session, company_id: int, office_location: str) -> Branch:
    name = office_location.strip()
    branch = (
        db.query(Branch)
        .filter(Branch.company_id == company_id, Branch.name.ilike(name))
        .first()
    )
    if branch:
        return branch
    code = unique_code(db, Branch, name)
    branch = Branch(company_id=company_id, name=name, code=code)
    db.add(branch)
    db.flush()
    return branch


def _get_or_create_department(db: Session, company_id: int, branch_id: int, department_name: str, head_user_id: int | None) -> Department:
    name = department_name.strip()
    department = (
        db.query(Department)
        .filter(Department.company_id == company_id, Department.branch_id == branch_id, Department.name.ilike(name))
        .first()
    )
    if department:
        if not head_user_id:
    ...
    department = Department(company_id=company_id, branch_id=branch_id, name=name, code=code, head_user_id=head_user_id)
    db.add(department)
    db.flush()
    return department
...
def sync_user_org_links(db: Session, user: User) -> None:
    ...
    Department (from department) under the single configured company.
    Silently does nothing if no company has been set up yet."""
    ...
    if user.office_location:
        branch = _get_or_create_branch(db, company.id, user.office_location)
        user.branch_id = branch.id

    if user.department and branch:
        _get_or_create_department(db, company.id, branch.id, user.department, user.reporting_manager_id)

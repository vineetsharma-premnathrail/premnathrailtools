...
**Sub-nav tabs**: modules with multiple pages get a hand-rolled nav component, e.g. `frontend/src/components/erp/ErpNav.tsx`, `frontend/src/components/p2p/P2PNav.tsx`. Each is `'use client'`, defines a local `TABS` array of `{ href, label, icon }`, an inline `TabIcon` SVG switch, uses `usePathname()` for active-tab detection (exact match for the module root, `startsWith` for sub-routes), and renders a flex row of `<Link>`s with a 2px bottom border on the active tab. There is **no shared generic `ModuleNav` component** — each module copies and adapts its own `<Module>Nav.tsx`; only the `TABS` array and accent color change. Follow this same copy-and-adapt approach for a new module's nav rather than trying to generalize it.

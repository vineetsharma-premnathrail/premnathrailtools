...
export default function QmaxPage() {
  ...
  const calculate = async () => {
    ...
    setError('')
    try {
      const data = await rndApi.calculateQmax(payload())
      ...
      setResult(data)
      rndApi.saveHistory({ tool_name: 'qmax', inputs: { ...payload(), e_param: eParam, q_applied: qApplied }, results: data.results, calculation_name: `Qmax d=${d}` }).catch(() => {})
    } catch {
